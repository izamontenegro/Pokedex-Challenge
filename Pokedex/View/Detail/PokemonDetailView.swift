...
struct PokemonDetailView: View {
    ...
    var body: some View {
        content
            ...
            .animation(.easeInOut, value: viewModel.teamFeedback)
            .task {
    ...
    private var statsView: some View {
        HStack {
            ForEach(PokemonStat.allCases, id: \.self) { stat in
                PokemonStatCard(
                    title: stat.title,
                    value: pokemon.statValue(for: stat)
                )
            }
        }
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: Theme.Spacing.s) {
                    ForEach(evolutions, id: \.self) { evolution in
                        Text(PokemonDataFormatter.name(evolution))
                            .font(Theme.Font.body)
                            .foregroundStyle(Theme.Color.primaryText)
                            .padding(.horizontal, Theme.Spacing.m)
                            .frame(height: 52)
                            .background(Theme.Color.forPokemonType(pokemon.types.first).opacity(0.4))
